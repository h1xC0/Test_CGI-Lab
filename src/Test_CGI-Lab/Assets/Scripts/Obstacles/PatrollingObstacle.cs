...
namespace Obstacles
{
	public class PatrollingObstacle : Obstacle
	{
		...
		void Update()
		{
			...
			var pingPongValue = Mathf.PingPong(m_CurrentPos, m_Segement.manager.laneOffset * k_LaneOffsetToFullWidth);
			var desiredDirection = m_Model.position;
			
			transform.localPosition = m_OriginalPosition - transform.right * pingPongValue;
			m_Model.LookAt(2 * m_Model.position - desiredDirection);
		}
	}
}
